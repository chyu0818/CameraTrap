...
class CameraTrapCropTripletDatasetNew(Dataset):
    def __init__(self, im_fp, file_lst_fn, annotations_fn, bbox_fn, percent_data, conf_threshold, transform=None):
        '''
        im_fp: filepath for images
        file_lst_fn: filename for list of images
        annotations_fn: filename for annotations
    ...
    def __getitem__(self, idx):
        ...
        return self.transform(im_crop), self.target_lst[idx]
...
class CameraTrapCropDatasetSimCLR(Dataset):
    def __init__(self, im_fp, file_lst_fn, annotations_fn, bbox_fn, percent_data, transform=None):
        '''
        im_fp: filepath for images
        file_lst_fn: filename for list of images
    ...
    def __getitem__(self, idx):
        # Load image.
        im_crop = Image.open(os.path.join(self.im_fp,'{}.jpg'.format(self.id_lst[idx])))
        return self.transform(im_crop), self.transform(im_crop), self.target_lst[idx]
